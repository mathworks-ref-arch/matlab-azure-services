classdef QueryV2ResponseUnparsedRows < adx.control.JSONMapper
    ...
    properties
        ...
        Rows string {adx.control.JSONMapper.doNotParse, adx.control.JSONMapper.fieldName(Rows,"Rows")}
        % DataSetCompletion
        HasErrors logical
        Cancelled logical
    ...
    methods
        % Constructor
        function obj = QueryV2ResponseUnparsedRows(s,inputs)
            ...
            arguments
                ...
                inputs.?adx.data.models.QueryV2ResponseUnparsedRows
            end
            obj@adx.control.JSONMapper(s,inputs);
        end


        % Should be called on the first frame in a QueryV2ResponseUnparsedRows only
        function dataSetHeader = getDataSetHeader(obj)
            if isprop(obj,  'FrameType')
                if strcmpi(obj.FrameType, "datasetheader")
                    ...
                    error("adx:QueryV2ResponseUnparsedRows","Expected first frame to be a DataSetHeader, found: %s", obj.FrameType);
                end
            else
                error("adx:QueryV2ResponseUnparsedRows","Expected first frame to the have a FrameType property");
            end
        end
        ...
        function dataSetCompletion = getDataSetCompletionFrame(obj)
            if isprop(obj, 'FrameType')
                if strcmpi(obj.FrameType, "dataSetCompletion")
                    ...
                    error("adx:QueryV2ResponseUnparsedRows","Expected last frame to be a DataSetCompletion, found: %s", obj.FrameType);
                end
            else
                error("adx:QueryV2ResponseUnparsedRows","Expected last frame to the have a FrameType property");
            end
        end
