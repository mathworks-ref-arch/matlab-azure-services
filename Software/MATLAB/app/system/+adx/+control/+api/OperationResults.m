classdef OperationResults < adx.control.BaseClient
    ...
    % Class methods
    methods
        function obj = OperationResults(options)
        ...
        function [code, result, response] = operationsResultsGet(obj, subscriptionId, location, operationId, api_version)
            ...
            arguments
              obj adx.control.api.OperationResults
            ...
            if ismember("application/json",specAcceptHeaders)
                ...
                error("control:api:operationsResultsGet:UnsupportedMediaType","Generated OpenAPI Classes only support 'application/json' MediaType.\n" + ...
                    "Operation '%s' does not support this. It may be possible to call this operation by first editing the generated code.","operationsResultsGet")
            end
            ...
            uri.EncodedPath = uri.EncodedPath + "/subscriptions/{subscriptionId}/providers/Microsoft.Kusto/locations/{location}/operationResults/{operationId}";

            % Substitute path parameters
            uri.Path(uri.Path == "{" + "subscriptionId" +"}") = subscriptionId;
            uri.Path(uri.Path == "{" + "location" +"}") = location;
            uri.Path(uri.Path == "{" + "operationId" +"}") = operationId;
            ...
            % Call preSend
            [request, httpOptions, uri] = obj.preSend("operationsResultsGet", request, httpOptions, uri);
            ...
            % Call postSend
            response = obj.postSend("operationsResultsGet", response, request, uri, httpOptions);

            % Handle response
            code = response.StatusCode;
            switch (code)
                case 200
                    result = adx.control.models.OperationResult(response.Body.Data);
                otherwise % Default output as specified in spec
                    result = adx.control.models.ErrorResponse(response.Body.Data);
            end
        
        end % operationsResultsGet method
