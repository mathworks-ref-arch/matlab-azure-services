classdef BlobSasPermission < azure.object
...
methods
    function obj = BlobSasPermission(varargin)
        if nargin == 0
            obj.Handle = com.azure.storage.blob.sas.BlobSasPermission();        
        elseif nargin == 1
            if isa(varargin{1}, 'com.azure.storage.blob.sas.BlobSasPermission')
                obj.Handle = varargin{1};
            else
                logObj = Logger.getLogger();
                write(logObj,'error','Expected argument of type com.azure.storage.blob.sas.BlobSasPermission');
            end                
        else
            logObj = Logger.getLogger();
